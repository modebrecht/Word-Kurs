...
def size_anchor(path: Path):
    reg,bold=resolve_font_paths(); W,H=1500,210
    im=Image.new('RGB',(W,H),'white'); d=ImageDraw.Draw(im)
    fl=ImageFont.truetype(bold,24); fb=ImageFont.truetype(bold,23); fs=ImageFont.truetype(reg,21)
    d.rounded_rectangle((18,18,W-18,H-18),radius=18,outline='#D3DEE2',width=3,fill='white')
    d.text((60,38),'Bild anklicken → Bildformat → Grösse',font=fl,fill='#1D6765')
    d.rounded_rectangle((700,40,1430,165),radius=12,outline='#AEBCC3',width=2,fill='#F8FAFB')
    d.text((740,70),'Breite',font=fb,fill='#17324D')
    d.rounded_rectangle((900,61,1120,112),radius=7,outline='#237B78',width=3,fill='#EAF4F3')
    d.text((948,72),'2,2 cm',font=fb,fill='#17324D')
    d.text((740,126),'Hier nur die Breite ändern.',font=fs,fill='#5E6D78')
    im.save(path,quality=95)


def build_document(out: Path, preview_path: Path, icon: Path, tools_path: Path):
...
def build(root: Path):
    ...
    icon=assets/'a5_school_icon.png'; image=prev/'a5_schulfest_vorlage.png'; tools=prev/'a5_bildgroesse_werkzeug.png'
    school_icon(icon); preview(image,icon); size_anchor(tools)
    build_document(sheets/'A5_Rette_das_Chaos_Dokument.docx',image,icon,tools)
